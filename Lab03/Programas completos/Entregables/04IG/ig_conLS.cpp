...
int main() {
    ...
    cout << "Iterated greedy con busqueda local" << endl;

    for (auto& instancia : Instancias) {
        cargar("flowshop/" + instancia);  // Carga los datos de la instancia
        int mk = NEH(ss);  // Aplica el algoritmo NEH
        mk = IG_conLS(ss);  // Aplica la búsqueda local con el makespan inicial de NEH
        cout << instancia << "\t" << mk << ": ";
